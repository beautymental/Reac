...
export function pickRandomWordFiltered(wordBank, difficulty, minLen=3, maxLen=14){
  const raw = wordBank?.[difficulty] ?? wordBank?.normal ?? ["REACT"];
  const list = raw.map((w)=>String(w).toUpperCase().trim())
    .filter((w)=> w.length >= Number(minLen||3) && w.length <= Number(maxLen||14));
  const finalList = list.length ? list : raw.map((w)=>String(w).toUpperCase().trim());
